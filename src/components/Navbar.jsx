...
const Navbar = () => {
    const Links = [
        ...
        { name: "Website Design & Development", link: "/GraphicsDesign" },
    ];
    const [open, setOpen] = useState(false);

    return (
        <div className='w-full relative z-50'>
            <div className='flex items-center justify-between bg-white py-4 md:py-6 md:px-10 px-6 shadow-md'>
                ...
                <ul className={`md:flex md:items-center md:pb-0 pb-4 absolute md:static bg-white md:z-auto z-40 left-0 w-full md:w-auto md:pl-0 pl-6 transition-all duration-500 ease-in ${open ? 'top-20' : '-top-32'} ${open ? 'flex-col md:flex-row' : 'hidden'}`}>
                    {
                        Links.map((link) => (
                            <li key={link.name} className='md:ml-6 md:mt-0 mt-2'>
                                <Link href={link.link} className='text-gray-800 hover:text-orange-600 duration-200 block md:inline-block font-semibold text-lg'>{link.name}</Link>
                            </li>
                        ))
                    }
